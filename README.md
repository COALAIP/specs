...
This section discusses a similar idea, namely the existence of an arbitrary complex vector used to link all the
identifiers of manifestations to the single identifier of a work on a global rights registry.
...
A special feature of this modeling is, as already mentioned, the distinction in "Works" (here typed as a `Creation`) and
"Manifestations" (typed as `Manifestation`). Both physical as well digital Manifestations could be represented, with the
difference that a digital Manifestation should be registered with a set of fingerprints as well as a link pointing to
(at least) an example of the work. We plan to subtype both `Creation` as well as `Manifestation` from schema.org's
CreativeWork so that extra properties can easily be added on case by case basis.
